...
export default function ErrorPage() {
  ...
  let message = "Sepertinya ada masalah yang tidak terduga. Silakan coba lagi."

  if (isRouteErrorResponse(error)) {
    status = error.status
    title = error.statusText || "Kesalahan"
    message = error.data?.message || `Terjadi kesalahan dengan status ${error.status}.`
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex items-center justify-center relative overflow-hidden">
      ...
      {/* Glow Effects */}
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-150 h-75 bg-destructive/10 rounded-full blur-[120px] opacity-40 pointer-events-none"></div>
      ...
      <div className="relative z-10 mx-auto max-w-2xl px-4 text-center">
        ...
        <div className="relative mb-8 h-48 sm:h-56 flex items-center justify-center">
          ...
          <div className="relative">
            <h1 className="text-[10rem] sm:text-[14rem] font-bold font-heading leading-none tracking-tighter bg-linear-to-br from-destructive/20 via-destructive/10 to-transparent bg-clip-text text-transparent select-none">
              {status}
            </h1>
          ...
          <div className="absolute top-0 right-0 sm:right-[-10%] p-3 rounded-2xl bg-card/70 backdrop-blur-xl border border-border/50 shadow-xl transform md:rotate-6 hover:rotate-0 transition-all duration-500 hover:scale-110 cursor-default">
            <p className="text-[9px] font-bold text-destructive uppercase tracking-wider mb-1">Error</p>
          ...
          <div className="absolute bottom-0 left-0 sm:left-[-5%] p-3 rounded-2xl bg-card/70 backdrop-blur-xl border border-border/50 shadow-xl transform md:-rotate-3 hover:rotate-0 transition-all duration-500 hover:scale-110 cursor-default">
            <p className="text-[9px] font-bold text-yellow-500 uppercase tracking-wider mb-1">Status</p>
            <div className="text-sm">
              <InlineMath math={`\\text{HTTP } ${status}`} />
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="space-y-3">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-destructive/10 border border-destructive/20 text-destructive text-xs font-semibold">
          ...
          <div className="inline-flex items-center gap-3 px-5 py-2.5 rounded-full bg-muted/50 border border-border/50 text-sm text-muted-foreground">
            <span className="text-destructive font-mono text-xs">lim</span>
            <span className="opacity-50">→</span>
            <span className="font-mono text-xs">error</span>
            <span className="opacity-30">=</span>
            <span className="font-mono text-xs">fail</span>
          </div>
