    ...
    <section id="team" className="mx-auto max-w-5xl px-4 py-20 sm:px-6 lg:px-8">
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-4">
        <div className="space-y-1">
          <h2 className="text-3xl font-bold tracking-tight font-heading">Tim Pengembang</h2>
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {teams.map((team) => (
          <SpotlightCard 
            key={team.id} 
            className="group p-4 border-border/40 bg-card rounded-4xl"
            spotlightColor="rgba(0, 120, 255, 0.1)"
          >
            <div className="relative aspect-square overflow-hidden rounded-full bg-muted mb-4 transition-all duration-500 group-hover:shadow-xl group-hover:shadow-primary/10">
              <img
                src={team.image}
                alt={team.name}
                className="w-xl h-full object-cover grayscale transition-all duration-700 ease-out group-hover:grayscale-0 group-hover:scale-105"
              />
            ...
            <div className="space-y-1 relative z-20 px-1">
              <div className="flex items-center justify-between gap-2">
                <h3 className="text-base font-bold font-heading group-hover:text-primary transition-colors duration-300 leading-tight">
                  {team.name}
                </h3>
                <a
                  href={team.instagram}
